...
use starmetal_core::publishing::PublishResult;
use starmetal_ops::{ConfigLoadOptions, ConfigOverrides, StarmetalRuntime, UpstreamOverride};
...
struct Cli {
    /// Path to config file. Defaults to STARMETAL_CONFIG, ./starmetal.toml, then built-in defaults.
...
enum ConfigAction {
    ...
    Init {
        ...
        #[arg(default_value = "starmetal.toml")]
        path: PathBuf,
    },
}
...
async fn run(cli: Cli) -> starmetal_core::error::Result<()> {
...
fn run_config(
    ...
    match action {
        ConfigAction::Show => {
            let config = starmetal_ops::load_config(options)?;
...
async fn run_cache(
    ...
    match action {
        CacheAction::DeleteArtifact { artifact, yes } => {
            if !yes {
                return Err(starmetal_core::error::StarmetalError::Config(
...
fn print_package_refs(
    packages: &[starmetal_ops::PackageRef],
    output: OutputFormat,
) -> starmetal_core::error::Result<()> {
    if matches!(output, OutputFormat::Json) {
...
fn write_artifact_output(path: Option<PathBuf>, data: &[u8]) -> starmetal_core::error::Result<()> {
