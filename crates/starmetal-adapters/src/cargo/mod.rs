...
async fn validate_index_metadata(
    ...
    for line in body.lines().filter(|line| !line.trim().is_empty()) {
        let entry: starmetal_core::registry::cargo::CargoIndexEntry = serde_json::from_str(line)
            .map_err(|err| StarmetalError::Storage(format!("invalid cached cargo index: {err}")))?;
