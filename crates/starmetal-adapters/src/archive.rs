...
pub(crate) fn parse_rubygem(data: &[u8]) -> Result<ArchiveMetadata> {
    ...
    let text = String::from_utf8(metadata)
        .map_err(|err| StarmetalError::Adapter(format!("invalid RubyGems metadata: {err}")))?;
    Ok(ArchiveMetadata {
...
pub(crate) fn parse_nuget(data: &[u8]) -> Result<ArchiveMetadata> {
    ...
    for index in 0..archive.len() {
        ...
        file.take(MAX_METADATA_BYTES as u64)
            ...
            .map_err(|err| StarmetalError::Adapter(format!("failed to read nuspec: {err}")))?;
        let text = String::from_utf8(buffer.clone())
            .map_err(|err| StarmetalError::Adapter(format!("invalid nuspec utf-8: {err}")))?;
...
pub(crate) fn parse_pub_archive(data: &[u8]) -> Result<ArchiveMetadata> {
    ...
    Ok(ArchiveMetadata {
        name: yaml_value(&value, "name")
            ...
            .ok_or_else(|| StarmetalError::Adapter("pubspec.yaml missing name".to_string()))?,
        version: yaml_value(&value, "version")
            .or_else(|| yaml_scalar(&text, "version"))
            .ok_or_else(|| StarmetalError::Adapter("pubspec.yaml missing version".to_string()))?,
        license: None,
        nuspec: None,
    })
}
...
pub(crate) fn parse_hex_tarball(data: &[u8]) -> Result<ArchiveMetadata> {
    ...
    let text = String::from_utf8(metadata)
        .map_err(|err| StarmetalError::Adapter(format!("invalid Hex metadata.config: {err}")))?;
    Ok(ArchiveMetadata {
        name: yaml_scalar(&text, "name")
...
fn gunzip(data: &[u8]) -> Result<Vec<u8>> {
    ...
    decoder
        .take(MAX_METADATA_BYTES as u64)
        .read_to_end(&mut buffer)
        .map_err(|err| StarmetalError::Adapter(format!("invalid gzip data: {err}")))?;
    Ok(buffer)
}
